...
/**
 * assigning the card number and display the next instruction
 */
  ngOnChanges(): void {
    if (this.card) {
      let cardNumber = this.assigningCardNumber();
      this.displayNextInstruction(cardNumber);
    }
  }
   ...
   *
   * @returns - the correct card number
   */
   assigningCardNumber(){
   ...
   * @param cardNumber
   */
  displayNextInstruction(cardNumber){
